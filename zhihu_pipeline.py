...
howmany=40
step=20
url_p1 = f'https://www.zhihu.com/api/v4/questions/{question_id}/answers?include=data[*].is_normal,admin_closed_comment,reward_info,is_collapsed,annotation_action,annotation_detail,collapse_reason,is_sticky,collapsed_by,suggest_edit,comment_count,can_comment,content,editable_content,attachment,voteup_count,reshipment_settings,comment_permission,created_time,updated_time,review_info,relevant_info,question,excerpt,is_labeled,paid_info,paid_info_content,relationship.is_authorized,is_author,voting,is_thanked,is_nothelp,is_recognized;data[*].mark_infos[*].url;data[*].author.follower_count,badge[*].topics;data[*].settings.table_of_content.enabled&limit={step}'
headers = {
...
jsons=[]
for offset in range(0,howmany,step):
    url_p2='&offset={}&platform=desktop&sort_by=default'.format(offset)
    url=url_p1+url_p2
...
for multiplier,data in enumerate(jsons):
    for row_idx, item in enumerate(data["data"]):
        ...
        for col_idx, stuff in enumerate(content):
            print("writing:", row_idx+multiplier*step + 1, col_idx, stuff)
            sheet1.write(row_idx+multiplier*step + 1, col_idx, stuff)
